...
개발자 Matthew (이광진)
시작일 2026.05.24
현재 단계 Sprint 5 완료 — 관리자 콘솔·회원 관리·캘린더 개선 진행 중
...
├── app/
│   ├── (auth)/login, signup
│   ├── (main)/coaches, dashboard, booking, sessions, coach/schedule, profile
│   ├── chat/[roomId]/
│   ├── admin/
│   │   ├── coaches/          ← 컨설턴트 승인 목록
...
│   ├── review/
│   └── ui/
│       ├── nav-buttons.tsx        ← [홈화면] [이전화면] 공통 버튼
│       ├── avatar-image.tsx       ← 프로필 사진 컴포넌트
│       └── shadcn/ui
├── lib/
